...
void Day4::Part1()
{
	...
	for (auto& line : data) {
		...
		if (regex_search(action, guard_match, guard_regex)) {
			int guardId = stoi(guard_match[2]);

			guardActions.push_back(GuardAction(guardId, DateTime(year, month, day, hour, minute), "begins shift"));
		}
		else {
	...
	cout << "Answer: " << longestSleeper.guardId * longestMinute << ", Guard ID: " << longestSleeper.guardId << ", Longest minute: " << longestMinute << endl;
}

void Day4::Part2()
{
	cout << "Day4 part 2" << endl;
